...
const ActorList = ({ id, getCredits }: { id: number, getCredits: (mediaId: number) => Promise<CreditsInfo> }) => {
  const displayNum = 9;
  const [creditsInfo, setCreditsInfo] = useState<CreditsInfo | null>(null);
  const displayPerson =
    creditsInfo?.cast && creditsInfo.cast.length > displayNum
      ? creditsInfo.cast.slice(0, displayNum)
      : creditsInfo?.cast;
  ...
  return (
    <>
      {actor && (
        ...
        >
          <Link to={`/person/${actor.id}`}>
            ...
            />
            <CardContent
              sx={{ mt: 1, padding: 0, paddingLeft: 2, marginBottom: 0 }}
            >
              <Typography
                variant="h6"
                component="h6"
      ...
      {!actor && (
        ...
        >
          <CardContent>
            <Link to={`/movie/movieId/cast`}>
              <Typography
                sx={{ fontWeight: "bold", ":hover": { color: "gray" } }}
              >
                查看更多 ➡️
              </Typography>
            </Link>
