...
import getMovieKeywords from "../api/getMovieKeywords";
import { type Keyword } from "../types/moive";

const MovieInfoSidebar = (props: {movieId: number, movieDetail: MovieDetail}) => {
        ...
        <>
            ...
            >
                <Box sx={{ marginBottom: 2, marginTop: 5 }}>
                    ...
                    <Typography component='h6' sx={{ color: "text.secondary" }}>{props.movieDetail.original_title}</Typography>
                </Box>

                <Box sx={{ marginBottom: 2 }}>
                    <Typography variant="h6">状态</Typography>
                    <Typography sx={{ color: "text.secondary" }}>{props.movieDetail.status === "Released"? "已上映": "未上映"}</Typography>
                </Box>

                <Box sx={{ marginBottom: 2 }}>
                    <Typography variant="h6">默认语言</Typography>
                    <Typography sx={{ color: "text.secondary" }}>{props.movieDetail.original_language.toUpperCase()}</Typography>
                </Box>

                <Box sx={{ marginBottom: 2 }}>
                    <Typography variant="h6">预算</Typography>
                    <Typography sx={{ color: "text.secondary" }}>${budgetFormatted}</Typography>
                </Box>

                <Box sx={{ marginBottom: 2 }}>
                    <Typography variant="h6">票房</Typography>
                    <Typography sx={{ color: "text.secondary" }}>${revenueFormatted}</Typography>
                </Box>

                <Box sx={{ marginBottom: 2 }}>
                    <Typography component='h6' variant="h6">关键词</Typography>
                    <Box
                        sx={{
                            display: 'flex',
                            width: "100%",
