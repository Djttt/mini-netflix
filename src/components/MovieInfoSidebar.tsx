...
const MovieInfoSidebar = (props: {
  ...
  const budgetFormatted = formatMoney(props.detail.budget);
  const revenueFormatted = formatMoney(props.detail.revenue);

  const [keywords, setKeywords] = useState<Keyword[]>([]);

  useEffect(() => {
    getMovieKeywords(props.Id).then((data) => {
      ...
      >
        ...
        <Box sx={{ marginBottom: 2 }}>
          ...
          <Typography sx={{ color: "text.secondary" }}>
            {props.detail.status === "Released" ? "已上映" : "未上映"}
          </Typography>
        </Box>
